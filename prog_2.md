...
# Data Structure
We use two stacks to solve this problem. The first stack is used in conversion of infix mathematical expression to postfix and the evalStack is used in the evaluation of the postfix mathematical expression. All the necessary stack functions are defined. 

# Functions:
## 1. checkExpression(char exp[]):
...
## 2. precedence(char c):
This function takes a character as its parameter. First, it checks if the character is '^' as it has the highest precedence and returns the value 3. Then, it checks if the character is '*' or '/' and returns 2. Then, it checks if the character is '+' or '-' and returns 1 else it returns -1.

## 3. evaluatePostfix(char* exp):
